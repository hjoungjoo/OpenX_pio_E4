...
#include "../../telescope/Telescope.h"
...
#if DEBUG != OFF || DEBUG_ECHO_COMMANDS != OFF
  ...
  #define L_CE_NULL                    "null"
  #define L_CE_UNK                     "unknown"

  static const char commandErrorStr[26][25] = {
  L_CE_NONE, L_CE_0, L_CE_CMD_UNKNOWN, L_CE_REPLY_UNKNOWN, L_CE_PARAM_RANGE, L_CE_PARAM_FORM,
  L_CE_ALIGN_FAIL, L_CE_ALIGN_NOT_ACTIVE, L_CE_NOT_PARKED_OR_AT_HOME, L_CE_PARKED,
  L_CE_PARK_FAILED, L_CE_NOT_PARKED, L_CE_NO_PARK_POSITION_SET, L_CE_SLEW_FAIL, L_CE_LIBRARY_FULL,
  L_CE_SLEW_ERR_BELOW_HORIZON, L_CE_SLEW_ERR_ABOVE_OVERHEAD, L_CE_SLEW_ERR_IN_STANDBY,
  L_CE_SLEW_ERR_IN_PARK, L_CE_SLEW_ERR_SLEW, L_CE_SLEW_ERR_OUTSIDE_LIMITS,
  L_CE_SLEW_ERR_HARDWARE_FAULT, L_CE_MOUNT_IN_MOTION, L_CE_SLEW_ERR_UNSPECIFIED, L_CE_NULL,
  L_CE_1
  };
  #define COMMAND_ERROR_LOGGABLE(e) ((e) > CE_0 && (e) != CE_1 && (e) != CE_NULL)
#endif

#ifndef COMMAND_ERROR_LOGGABLE
  #define COMMAND_ERROR_LOGGABLE(e) false
#endif
...
void CommandProcessor::poll() {
  ...
  if (buffer.ready()) {
    ...
    // debug, log errors and/or commands
      ...
      if (DEBUG_ECHO_COMMANDS == ON || COMMAND_ERROR_LOGGABLE(commandError)) {
    ...
    if (commandError != CE_NULL) {
      ...
      #if DEBUG_ECHO_COMMANDS != OFF
        if (COMMAND_ERROR_LOGGABLE(commandError)) { DF(", Error "); D(commandErrorStr[commandError]); }
      #endif
    }
    #if DEBUG_ECHO_COMMANDS != OFF
      if (DEBUG_ECHO_COMMANDS == ON || COMMAND_ERROR_LOGGABLE(commandError)) { DL(""); }
    #endif
    #ifdef DEBUG_ECHO_COMMANDS_CH
      }
    #endif
