...
CommandError Goto::validate() {
  if (!axis1.isEnabled()) {
    DLF("WRN: Goto.validate, axis1 disabled");
    return CE_SLEW_ERR_IN_STANDBY;
  }
  if (!axis2.isEnabled()) {
    DLF("WRN: Goto.validate, axis2 disabled");
    return CE_SLEW_ERR_IN_STANDBY;
  }
  if (park.state == PS_PARKED) {
    DLF("WRN: Goto.validate, mount parked");
    return CE_SLEW_ERR_IN_PARK;
  }
  if (state != GS_NONE) {
    DF("WRN: Goto.validate, goto busy state="); D(state); DF(" stage="); DL(stage);
    return CE_SLEW_IN_SLEW;
  }
  if (guide.state != GU_NONE) {
    DF("WRN: Goto.validate, guide busy state="); DL(guide.state);
    return CE_SLEW_IN_MOTION;
  }
  if (mount.isSlewing()) {
    DLF("WRN: Goto.validate, mount slewing");
    return CE_SLEW_IN_MOTION;
  }
  if (limits.isGotoError()) {
    DLF("WRN: Goto.validate, goto limits error");
    return CE_SLEW_ERR_OUTSIDE_LIMITS;
  }
  if (mount.motorFault()) {
    DLF("WRN: Goto.validate, motor fault");
