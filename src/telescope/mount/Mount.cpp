...
#ifndef TRACKING_RATE_SAFETY_LIMIT
  #define TRACKING_RATE_SAFETY_LIMIT TRACK_BACKLASH_RATE
#endif
...
static float limitTrackingRate(float rate, uint8_t axisNumber) {
  if (isnan(rate) || isinf(rate)) return 0.0F;

  const float limit = (float)TRACKING_RATE_SAFETY_LIMIT;
  if (limit <= 0.0F) return rate;

  if (rate > limit || rate < -limit) {
    #if DEBUG != OFF
      static unsigned long lastWarnMs[2] = {0, 0};
      uint8_t index = axisNumber > 0 && axisNumber <= 2 ? axisNumber - 1 : 0;
      unsigned long now = millis();
      if (lastWarnMs[index] == 0 || (long)(now - lastWarnMs[index]) >= 5000) {
        DF("WRN: Mount, tracking axis"); D(axisNumber);
        DF(" rate limited raw="); D(rate);
        DF(" limit="); DL(limit);
        lastWarnMs[index] = now;
      }
    #endif
    return rate > 0.0F ? limit : -limit;
  }
...
void Mount::update() {
  ...
  if (guide.state < GU_GUIDE) {
    ...
    if (!guide.activeAxis1() || guide.state == GU_PULSE_GUIDE) {
      ...
      f1 = limitTrackingRate(f1, 1);
      #if DEBUG != OFF
        static uint8_t trackLogCount1 = 0;
        if (trackingState == TS_SIDEREAL && trackLogCount1 < 8) {
          DF("DBG: Mount.update axis1 f="); D(f1);
          DF(" sync="); DL(siderealToRadF(f1)*SIDEREAL_RATIO_F*site.getSiderealRatio());
          trackLogCount1++;
        }
      #endif
    ...
    if (!guide.activeAxis2() || guide.state == GU_PULSE_GUIDE) {
      ...
      f2 = limitTrackingRate(f2, 2);
      #if DEBUG != OFF
        static uint8_t trackLogCount2 = 0;
        if (trackingState == TS_SIDEREAL && trackLogCount2 < 8) {
          DF("DBG: Mount.update axis2 f="); D(f2);
          DF(" sync="); DL(siderealToRadF(f2)*SIDEREAL_RATIO_F*site.getSiderealRatio());
          trackLogCount2++;
        }
      #endif
