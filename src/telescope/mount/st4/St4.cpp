...
#include "../status/Status.h"
  ...
  #include "../../../lib/pushButton/PushButton.h"
    ...
    #ifndef ST4_SHC_TONE_LOSS_MS
      #define ST4_SHC_TONE_LOSS_MS 1500UL
    #endif
  #else
  ...
  void St4::poll() {
    ...
    static unsigned long shcToneLostStartMs = 0;
    ...
    if (!shcActive) {
      ...
      if (shcActive) {
        ...
        if (st4Axis1Rev.hasTone()) {
          shcToneLostStartMs = 0;
        } else {
          if (shcToneLostStartMs == 0) shcToneLostStartMs = millis();
        }

        if (shcToneLostStartMs != 0 && (uint32_t)(millis() - shcToneLostStartMs) >= ST4_SHC_TONE_LOSS_MS) {
