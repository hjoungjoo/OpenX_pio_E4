...
void Axis::setFrequency(float frequency) {
  ...
  float safeScaleFreq = scaleFreq;
  if (isnan(safeScaleFreq) || isinf(safeScaleFreq)) safeScaleFreq = 1.0F;
  frequency *= safeScaleFreq;
  if (isnan(frequency) || isinf(frequency)) frequency = 0.0F;

  float safeBaseFreq = baseFreq;
  if (isnan(safeBaseFreq) || isinf(safeBaseFreq)) safeBaseFreq = 0.0F;

  if (powerDownStandstill && frequency == 0.0F && safeBaseFreq == 0.0F) {
    if (!poweredDown) {
  ...
  if (enabled) {
    float effectiveFrequency = frequency;
    if (autoRate == AR_NONE) {
      effectiveFrequency += safeBaseFreq;
    }
    if (isnan(effectiveFrequency) || isinf(effectiveFrequency)) effectiveFrequency = 0.0F;
    if (maxFreq != 0.0F) {
      if (effectiveFrequency > maxFreq) effectiveFrequency = maxFreq; else
      if (effectiveFrequency < -maxFreq) effectiveFrequency = -maxFreq;
    }
    double motorFrequency = effectiveFrequency*stepsPerMeasureValue;
    if (isnan(motorFrequency) || isinf(motorFrequency)) motorFrequency = 0.0;
    motor->setFrequencySteps((float)motorFrequency);
  } else {
